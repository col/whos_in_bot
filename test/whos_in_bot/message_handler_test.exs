defmodule WhosInBot.MessageHandlerTest do
  ...
  test "/whos_in lists 'in' people in correct order", %{ roll_call: roll_call } do
    Repo.insert!(%RollCallResponse{ roll_call_id: roll_call.id, status: "in", user_id: 2, name: "User 2", inserted_at: ~N[2015-02-02 02:02:02]})
    Repo.insert!(%RollCallResponse{ roll_call_id: roll_call.id, status: "in", user_id: 1, name: "User 1", inserted_at: ~N[2015-01-01 01:01:01]})
    Repo.insert!(%RollCallResponse{ roll_call_id: roll_call.id, status: "in", user_id: 3, name: "User 3", inserted_at: ~N[2015-03-03 03:03:03]})
    {status, response} = MessageHandler.handle_message(message(%{text: "/whos_in"}))
    assert {status, response} == {:ok, "1. User 1\n2. User 2\n3. User 3\n"}
  end
  ...
  test "/whos_in lists 'out' people in correct order", %{ roll_call: roll_call } do
    Repo.insert!(%RollCallResponse{ roll_call_id: roll_call.id, status: "out", user_id: 2, name: "User 2", inserted_at: ~N[2015-02-02 02:02:02]})
    Repo.insert!(%RollCallResponse{ roll_call_id: roll_call.id, status: "out", user_id: 1, name: "User 1", inserted_at: ~N[2015-01-01 01:01:01]})
    Repo.insert!(%RollCallResponse{ roll_call_id: roll_call.id, status: "out", user_id: 3, name: "User 3", inserted_at: ~N[2015-03-03 03:03:03]})
    {status, response} = MessageHandler.handle_message(message(%{text: "/whos_in"}))
    assert {status, response} == {:ok, "Out\n - User 1\n - User 2\n - User 3\n"}
  end
